...
public class InsuranceCarrierBreakdownReader : IInsuranceCarrierBreakdownReader
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Calculate total commission across all carriers for percentage calculations
        var totalCommission = filteredStatements.Sum(s => s.CommissionAmount ?? 0);

        // Group statements by carrier and calculate metrics for each carrier
        var carrierBreakdowns = filteredStatements
            ...
            .Select(g =>
            {
                ...
                return new AgentCarrierBreakdown
                {
                    ...
                    // Calculate the percentage of total commission this carrier represents
        ...
        // Load all carriers from the database
        var carriers = await _context.Carriers
            .AsNoTracking()
            .ToListAsync(cancellationToken);

        // Add carriers with no commission data (0 values) to ensure complete breakdown
        // This shows all available carriers, even if the agent has no business with them
